...
| 在收到数据的 14 ms 内解码，计算接收频率，接收频率大于 71.4 Hz，且无丢包 | 6% |
| 数据检验和校对 | 7% |
...
- 将完成后的代码压缩（整个文件夹，把 `build` 文件夹删掉）并在 Canvas 上提交
...
最终提交的截止时间为 **2026 年 10 月 1 日 22:00:00**。仅在 Canvas 上提交代码视为无效提交，请在实验室做完后向我们的 TA 展示效果，并向 TA 确认完成情况后再提交代码。

### 延迟提交政策

每迟交一分钟将被扣 1 分（最高 100 分）。例如，由于作业 1 的截止时间是 10 月 1 日 22:00:00，如果您在 10 月 1 日 23:00:00 提交作业，您的作业将被扣 60 分。但是，您作业的最低分数为零分：扣除迟交罚款（以及其他任何罚款）后的任何负分将被重置为零分。（与 CSE 部门相同）
